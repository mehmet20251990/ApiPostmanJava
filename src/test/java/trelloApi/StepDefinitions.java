...
public class StepDefinitions {
    ...
    public void kullanıcı_board_create_edebilmek_icin_ilgili_url_e_POST_methodu_ve_ilgili_endpointler_ile_request_atar
                                                        (String idPath, String id, String boardsPath, String boards) {
        /*
        specification.pathParams("idPath",1,
                "boardsPath","boards");
        */
        //https://api.trello.com/1/boards/?name={name}&key=APIKey&token=APIToken'
        specification.pathParams(idPath,id, boardsPath,boards).
                queryParams("name",trelloApiTestData.getBoardName(), "key",trelloApiTestData.getKey(),
    ...
    public void kullanıcı_list_create_edebilmek_icin_ilgili_url_e_POST_methodu_ile_request_atar(String idPath, String id,
                                                                                    String listsPath, String list) {
        //#https://api.trello.com/1/lists?name={name}&idBoard=5abbe4b7ddc1b351ef961414&key=APIKey&token=APIToken
        specification.pathParams(idPath,id, listsPath,list).
    ...
    public void kullanıcı_card_ccreate_edebilmek_icin_ilgili_url_e_POST_metodu_ile_request_atar(String idPath, String id,
                                                                             String cardsPath, String cards) {
        ...
        specification.pathParams(idPath,id, cardsPath,cards).
                queryParams("idList",listID, "name","CwCARDS", "key", trelloApiTestData.getKey(),
                        "token",trelloApiTestData.getToken());
        String idPathFormated = String.format("{%s}",idPath);
    ...
    public void kullanıcı_update_islemi_iicn_ilgili_url_ePUT_metodu_ile_request_atar(String idPath, String id,
                                                                                     String cardsPath, String cards) {
        //https://api.trello.com/1/cards/{id}?key=APIKey&token=APIToken
        Random random = new Random();
        int numberOfRandom = random.nextInt(cardsID.size()); //Bu metot nextInt() in içerisne verileen bir paramternin
                                                    // 0 ile o değer arasında random bir sayı generate eder. [0,value)
        specification.pathParams(idPath,id, cardsPath,cards, "cardIdPath",cardsID.get(numberOfRandom)).
                queryParams("idList",listID, "name","Cw Card Update", "key",trelloApiTestData.getKey(),
                        "token",trelloApiTestData.getToken());
        String idPathFormated = String.format("{%s}",idPath);
        String cardsPathFormated = String.format("{%s}",cardsPath);
    ...
    public void kullanıcı_delete_islemi_yapabilmek_icin_ilgili_url_e_DELETE_metodu_ile_request_atar(String idPath, String id,
                                                             String cardsPath, String cards, Integer cardsIdGET) {
    }
    ...
    public void kullanıcı_delete_islemi_yapabilmek_icin_ilgili_url_e_DELETE_metodu_ile_request_atar(String idPath,
          String id, String cardsPath, String cards, String cardIdPath, Integer cardsIdGET) throws InterruptedException {
        //#"https://api.trello.com/1/cards/{id}?key=APIKey&token=APIToken"
        specification.pathParams(idPath,id, cardsPath,cards, cardIdPath,cardsID.get(cardsIdGET)).
    ...
    public void kullanıcı_board_delete_islemi_icin_ilgili_url_e_DELETE_metodu_ile_request_atar(String idPath,
                                                                      String id, String boardsPath, String boards) {
        specification.pathParams(idPath,id, boardsPath,boards, "boardIdePath",boardID).
                queryParams("key",trelloApiTestData.getKey(), "token",trelloApiTestData.getToken());
        String idPathFormated = String.format("{%s}",idPath);
        String boardsPathFormated = String.format("{%s}",boardsPath);
        response = given().spec(specification).contentType(ContentType.JSON).when().delete("/" + idPathFormated + "/" +
                                                                            boardsPathFormated + "/{boardIdePath}");
    }
